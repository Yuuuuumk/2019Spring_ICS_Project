...
class Server:
    ...
    def get_clients(self):
        try:
          self.mem_file = open('members','rb')
          self.members = pickle.load(self.mem_file)
          self.mem_file.close()
        except:
          self.mem_file = open('members','wb')
          pickle.dump(self.members, self.mem_file)
          self.mem_file.close()
        ...
        try:
            ...
            if len(msg) > 0:
                if msg["action"] == "register":
                    ...
                    self.add_client(name, password)
                    print(msg)
                    print(self.members)
                    mysend(sock, json.dumps({"action":"register", "status":"register_success"}))
                ...
                if msg["action"] == "login":
                    ...
                    if name in self.members:
                      if self.members[name] == password:
                        ...
                        mysend(sock, json.dumps({"action":"login", "status":"user_nonexist"}))
                else:
...
#==============================================================================
                ...
                term = msg['target']
                from_name = self.logged_sock2name[from_sock] # get search search_rslt
